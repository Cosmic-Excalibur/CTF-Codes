...
def ramanujan_polynomial(D, cached = 1):
    ...
    pol = R(1)
    Rn = [
        lambda tau: eta(3*tau)*eta(tau/3)/eta(tau)**2,
        lambda tau: eta(3*tau)*eta(tau/3+1/3)/eta(tau)**2,
        lambda tau: eta(3*tau)*eta(tau/3+2/3)/eta(tau)**2,
        lambda tau: eta(tau/3)*eta(tau/3+2/3)/eta(tau)**2,
        lambda tau: eta(tau/3)*eta(tau/3+1/3)/eta(tau)**2,
        lambda tau: eta(tau/3+2/3)*eta(tau/3+1/3)/eta(tau)**2
    ]
    ...
    Ln = lambda n, a, b, c: (matrix([[a,(b-1)/2],[0,1]]) if a % n != 0 else (matrix([[-(b+1)/2,-c],[1,0]]) if c % n != 0 else matrix([[-(b+1)/2-a,(1-b)/2-c],[1,-1]]))).change_ring(R)
    ...
    for qf in tqdm(rqf):
        ...
        S = [S0, S1, S2, S3]
        T = [None, None, T2, T3]
        An = lambda n: (S[n]*T[n]**Zmod(N(n))(-1/a)*S[n]*T[n]**Zmod(N(n))(-a)*S[n]*T[n]**Zmod(N(n))((b-1)/2/a**2-1/a) if a % n != 0 else (T[n]**Zmod(N(n))(1-(b+1)/2)*ST_squared[n] if c % n != 0 else T[n]**Zmod(N(n))(1-(b+1)/2-a)*ST[n]*S[n]*T[n]**Zmod(N(n))(1-1/(a+b+c)))).change_ring(R)
        A = An(2)*An(3)*B
        tau = (-b+Dsqrt)/2/a
        if cached:
